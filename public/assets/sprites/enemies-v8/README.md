...
The game uses one RGBA atlas for each biome. Every atlas has 11 rows and four
320 px frames per row. `manifest.json` fixes the row order and runtime layout.

## Sources

Imported source strips live at `_src/strips-biome/<source-mood>/<enemy>.jpg`.
...
frames against one shared box, and writes the biome atlases. Its detailed run
