...
describe("environment bind adapters", () => {
  test("WebGPU PMREM is a lazy import and WebGL uses THREE.PMREMGenerator", async () => {
    const bind = await Bun.file(new URL("../src/systems/EnvironmentBind.ts", import.meta.url)).text();
    const lighting = await Bun.file(new URL("../src/systems/LightingRig.ts", import.meta.url)).text();
