...
describe("floor campfire assembly", () => {
  test("image-sculpted campfire has stone ring, logs, layered flame and radial light", () => {
    ...
    expect(fire.flame.geometry.name).toBe("Authored low-poly flame tongue");
    expect(fire.flame.geometry).not.toBeInstanceOf(THREE.OctahedronGeometry);
    ...
    expect((fire.flame.material as THREE.MeshBasicMaterial).toneMapped).toBe(true);
